...
AotTensor* aot_tensor_create(const float* data, const int64_t* shape, size_t ndim,
    ...
    try {
        ...
        if (device.is_cpu()) {
            ...
            torch::Tensor tensor = torch::tensor(data_owned, torch::kFloat32).reshape(shape_vec);
            return new AotTensor(std::move(tensor));
        }

        // GPU: Create empty tensor and copy data
        // 1. Create empty GPU tensor
        auto gpu_options = torch::TensorOptions().dtype(torch::kFloat32).device(device);
        torch::Tensor tensor = torch::empty(shape_vec, gpu_options);

        // 2. Create CPU tensor using from_blob on owned data
        torch::Tensor cpu_tensor = torch::from_blob(data_owned.data(), shape_vec,
...
AotResult* aot_model_infer(AotModel* model, AotTensor* input) {
    ...
    try {
        // Get input tensor - should already be on correct device from aot_tensor_create
        ...
        DEBUG_PRINT("[DEBUG] Input tensor - mean: %.6f, std: %.6f, device: %s\n",
                    input_tensor.mean().item<float>(), input_tensor.std().item<float>(),
...
int aot_test_device(const char* device_str) {
    try {
        ...
        for (const auto& [dtype, name] : dtypes) {
            try {
                auto x = torch::randn({4, 4}, torch::TensorOptions().device(device).dtype(dtype));
                results += name + ": OK\n";
            } catch (const std::exception& e) {
        ...
        for (const auto& [dtype, name] : dtypes) {
            try {
                auto x = torch::empty_strided({2}, {1},
        ...
        for (const auto& [dtype, name] : dtypes) {
            try {
                auto x =
                    torch::randn({4, 128, 768}, torch::TensorOptions().device(device).dtype(dtype));
                auto y =
                    torch::randn({4, 128, 768}, torch::TensorOptions().device(device).dtype(dtype));
